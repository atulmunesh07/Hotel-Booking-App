            ...
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <Link to={'/'} className="flex items-center gap-2">
                    <img src="https://static.vecteezy.com/system/resources/thumbnails/047/247/050/small_2x/hotel-3d-illustration-png.png" alt="logo"
                        className='w-15'
                    />
                    <span className={` font-bold text-xl ${isScrolled ? "text-black" : "text-white"} `}>HotelBooking.com</span>
                </Link>
                ...
                <div className="hidden md:flex items-center gap-4">
                    <img src={assets.searchIcon} alt="search logo" className={`${isScrolled && 'invert'} h-7 transition duration-150`} />


                    {user ? (<UserButton>
                        <UserButton.MenuItems>
                            <UserButton.Action label='My Booking'
                                labelIcon={<BookIcon></BookIcon>} onClick={() => navigate('/my-booking')}
                            ></UserButton.Action>
                        </UserButton.MenuItems>
                    </UserButton>) :
                        (
                ...
                <div className="flex items-center gap-3 md:hidden">
                    ...
                    <img onClick={() => setIsMenuOpen(!isMenuOpen)}
                        ...
                        src={assets.menuIcon} alt="menu logo" />
                </div>

                {/* Mobile Menu */}

                <div className={`fixed top-0  left-0 w-full h-screen backdrop-blur-2xl bg-white/30 
                    ...
                    <button className="absolute top-4 right-4" onClick={() => setIsMenuOpen(false)}>
                        <img src={assets.closeIcon} alt="logo" className='h-6.5' />
                    </button>
